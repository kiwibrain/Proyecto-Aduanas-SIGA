...
import { useAccessibility, T } from "@/lib/accessibility";
...
export function Layout({ children, fullWidth = false }: { children: React.ReactNode; fullWidth?: boolean }) {
  ...
  const rolLabel: Record<string, string> = {
    viajero: language === "es" ? "Viajero" : "Traveler",
    aduana: "Aduana",
    pdi: "PDI",
    sag: "SAG",
  };
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans">
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center space-x-1 text-sm font-medium">
            ...
            {user ? (
              ...
              <>
                <Link href="/login" className="px-3 py-2 rounded-md hover:bg-blue-50 text-gray-700 hover:text-[#0032A0] transition-colors">{t.login}</Link>
      ...
      <div className="bg-[#00205B] text-white py-1.5 px-4 hidden md:flex items-center justify-center gap-6 text-xs">
        <span className="badge-connected">PDI {language === "es" ? "Integrado" : "Integrated"}</span>
        <span className="badge-connected">SAG {language === "es" ? "Integrado" : "Integrated"}</span>
        <span className="badge-connected">{language === "es" ? "Registro Civil" : "Civil Registry"}</span>
      ...
      <footer role="contentinfo" className="bg-[#00205B] text-white py-8 mt-auto">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <p className="font-bold text-white">{t.customs}</p>
              <p className="text-white/70 text-sm mt-1">{t.ministry}</p>
            </div>
            ...
            <div className="text-right">
              <p className="text-white/70 text-xs">{t.gov}</p>
